...
2. **`DEV_LOG.md` (개발 일지)**  
   ...
   - **최신 날짜가 `DEV_LOG.md` 맨 위**에 오도록 한다. 오늘 섹션이 없으면 상단(첫 날짜 구분선 아래)에 긴 점선 + `## YYYY.MM.DD`를 **삽입**한다. 있으면 그 섹션에만 항목 추가.
   - **`DEV_LOG.md`만** 저장 서식 예외: `.vscode/settings.json`(plaintext + `[plaintext]` formatOnSave 끔), `.prettierignore`에 `DEV_LOG.md`만 등록.  
...
- `DEV_LOG.md` 작성 형식: **`번호. 제목`** → 요약 불릿 한 줄 → **`수정 전` / `수정 후`**(바꾼 경우; 신규만이면 `추가`로 대체). 제목과 디테일 사이 빈 줄 없음.
